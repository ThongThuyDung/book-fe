...
//   useEffect(() => {
//     // Fetch initial chatbot message or perform any necessary initialization
//     fetchChatbotMessage('Hello');
//   }, []);

  const fetchChatbotMessage = async (input) => {
    // Make an API call to the ChatGPT API with the user input
    const response = await fetch('http://35.197.157.86:6789/api/test/chat?prompt=' + input, {
      method: 'GET',
      headers: {
    ...
    //   body: JSON.stringify({
    //     // messages: [{ role: 'system', content: 'You are a user' }, { role: 'user', content: input }],
    //     prompt: input
    //   }),
    });
  ...
  return (
    ...
    <div className='main'>
      <div className='icon_main'>
        <img onClick={()=> setOpenChat(!openChat)} alt='' className='icon' src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSDcm7kiokRTfw0s-dbrAk2P_aMjFGQnx9UfkLfmiHvADa9I4DuHzaLu8DNUzTGjqTGZds&usqp=CAU' />
       </div>
       {openChat && (
       <div className='chat'>
